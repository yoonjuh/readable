...
const Button = styled.div`
  ...
  border: ${({ category, currentCat }) =>
    category === currentCat ? "" : "0.05rem black solid"};
  color: ${({ category, currentCat }) =>
    category === currentCat ? "white" : "black"};
  background-color: ${({ category, currentCat }) =>
    category === currentCat ? "rgba(231,0,65, 0.7)" : "white"};
  box-shadow: ${({ category, currentCat }) =>
    category === currentCat ? "0 0 1.5rem rgba(231,0,65, 0.5)" : ""};
  font-weight: 400;
  :hover {
    background-color: ${({ category, currentCat }) =>
      category === currentCat ? "#dc3545" : "rgba(55, 60, 66, 0.8)"};
    color: white;
    transition: 0.7s;
  }
`;
